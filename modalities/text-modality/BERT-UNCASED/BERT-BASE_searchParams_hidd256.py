...
from transformers import AutoModel, AutoTokenizer, BertTokenizerFast, AdamW
...
subjob_name = 'hidd256'
# Model name
model_name = 'BERT-UNCASED'
# Base path
base_path = '/home/k/kyparkypar/ondemand/data/sys/myjobs/projects/default/text-modality/BERT-UNCASED/'
# Log file path
...
class BERT_Arch(nn.Module):
    def __init__(self, bert, dropout_p=0.1, hidden_size=768, activation_fn='No'):
        ...
        # FC Layer
        self.fc = nn.Linear(768, hidden_size)  # BERT outputs 768 by default
        # Output layer
        # MULTILABEL CLASSIFICATION
    ...
    def forward(self, sent_id, mask):
        ...
        cls_hs = outputs[1]                            # CLS token (only for BERT)
        # cls_hs = outputs.last_hidden_state[:, 0, :]  # CLS token is at index 0 in the sequence (for other models)
        # ----------------------------------------------------------------
...
def train(model, train_dataloader):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        #     write_log(f'    Batch {step} of {len(train_dataloader)} - Loss: {loss.item():.4f}')

        total_loss += loss.item()

        # Backward pass and gradient clipping
        # We clip the gradients to prevent the model from exploding
        # and then we normalize the gradients to 1.0 for stable training
        loss.backward()
        ...
        # Moved to CPU because operations on tensors cannot be performed on cuda tensors so you need to move them to cpu first.
...
for params in ParameterGrid(param_grid):
    ...
    for train_index, val_index in kf.split(train_seq):
        ...
        # bert = AutoModel.from_pretrained('microsoft/deberta-base')

        model = BERT_Arch(bert, params['dropout_p'], params['hidden_size'], params['activation_fn'])
        ...
        for epoch in range(params['epochs']):
            ...
            train_loss, train_preds = train(model, train_dataloader)
            ...
            if val_loss <= best_valid_loss:   # If we find one with the same, keep the one with the biggest epoch
                ...
                # Convert predictions to class labels for both training and validation
                # MULTILABEL CLASSIFICATION
                ...
                train_accuracy = np.mean(train_labels == train_y_fold.numpy())
